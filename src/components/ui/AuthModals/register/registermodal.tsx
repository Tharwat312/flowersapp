...
export function RegisterModal({
    ...
    const Schema = z.object({
        firstName: z.string({ required_error: t('fname-required') }).min(1, t('fname-required')).max(10, t('name-long')),
        lastName: z.string({ required_error: t('fname-required') }).min(1, t('fname-required')).max(10, t('name-long')),
        ...
        gender: z.string().trim().refine(value => value.toLowerCase() === 'male' || value.toLowerCase() === 'female', {
            message: t('gender-invalid'),
        }).transform(value => value.toLowerCase().trim()),
        phone: z.string().regex(
            /^(\+2)01\d{9}$/, // Regex pattern
